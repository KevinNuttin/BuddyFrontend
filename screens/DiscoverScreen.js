...
export default function App(props) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      arraytemp = arraytemp.filter(
        (e) =>
          e.pseudo != dataMyProfil.user.pseudo &&
          e.langues != dataMyProfil.user.langue
      );
  ...
  async function handleYup(card) {
    ...
    const data = await fetch("http://192.168.10.145:3000/match/like", {
      method: "PUT",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      ...
      <View style={styles.container}>
        {cards ? (
          <SwipeCards
            ...
            renderNoMoreCards={() => (
              <StatusCard text="Pas de Buddy, pas de partie..." />
            )}
            actions={{
